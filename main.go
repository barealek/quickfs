...
func sendReceiversUpdate(upload *Upload) {
	...
	safeReceivers := make([]map[string]interface{}, len(receivers))
	for i, r := range receivers {
		safeReceivers[i] = map[string]interface{}{
			"id":           r.ID,
			"name":         r.Name,
...
func main() {
	...
	router.HandleFunc("/{path:join}", func(w http.ResponseWriter, r *http.Request) {
		http.ServeFile(w, r, *staticDir+"/index.html")
	}).Methods("GET")
